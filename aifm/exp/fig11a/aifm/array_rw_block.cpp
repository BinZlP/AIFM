...
#define ARRAY_ENTRY_NUM 1280
#define BLOCK_NUM 32
#define BLOCK_ENTRY_NUM ((ARRAY_ENTRY_NUM*(1<<20))/BLOCK_NUM)
#define BLOCK_SIZE 8*BLOCK_ENTRY_NUM
...
typedef struct {
  ...
  uint64_t arr[BLOCK_ENTRY_NUM];
}ArrBlock;

//constexpr uint64_t kNumEntries = 1280 * (1 << 20);
//constexpr uint64_t kArrayBlockSizeMB = 128;
//constexpr uint64_t kArrayBlockSize = kArrayBlockSizeMB * (1 << 20);
...
alignas(BLOCK_SIZE) ArrBlock array_block;
std::unique_ptr<Array<ArrBlock, BLOCK_NUM>> array_ptrs[BLOCK_NUM];
...
/*  for (uint32_t i = 0; i < kNumUncompressedFiles; i++) {
  ...
  for (uint32_t i = 0; i < BLOCK_NUM; i++) {
    array_ptrs[i].reset(manager->allocate_array_heap<ArrBlock, BLOCK_NUM>());
  }
