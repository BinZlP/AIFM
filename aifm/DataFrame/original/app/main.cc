...
const std::array<const char *,19> col_names = { /* int */ "VendorID", "passenger_count", "RatecodeID", "RatecodeID",
                                                /* SimpleTime */ "tpep_pickup_datetime", "tpep_dropoff_datetime",
                                                /* double */ "trip_distance", "pickup_longitude", "pickup_latitude", "dropoff_longitude", 
...
void write_all_zero(StdDataFrame<uint64_t>& df)
{
    ...
    for(int i=0; i<=3; i++)
    {
        std::cout << "Writing columns: " << col_names[i] << std::endl;
    ...
    {
        std::cout << "Writing columns: " << col_names[i] << std::endl;
        auto v = df.get_column<double>(col_names[i]);
        auto it = v.begin();
    ...
    {
        std::cout << "Writing columns: " << col_names[16] << std::endl;
...
void traverse_all(StdDataFrame<uint64_t>& df)
{
    ...
    for(int i=0; i<=3; i++)
    {
        ...
        auto it = v.begin();
        auto val = *it;
        for(int j=0; j<v.size(); j++, it++) val = *it;
    }
